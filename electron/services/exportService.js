...
export async function exportFactures(payload = {}) {
  ...
  if (format === 'pdf') {
    ...
    const doc = createPdfDocument(
      ...
      `Edition du ${formatDate(new Date())}`,
    )
    ...
    drawMetricCard(doc, 136, 42, 60, 'Factures payees', String(factures.filter((item) => item.statut === 'PAYEE').length))
