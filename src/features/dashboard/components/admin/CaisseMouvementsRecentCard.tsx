// src/features/dashboard/components/admin/CaisseMouvementsRecentCard.tsx

/**
 * @module features/dashboard/components/admin/CaisseMouvementsRecentCard
 * @description
 ...
 * - **Avatar illustratif** : icône personnalisée (entrée/sortie) avec dégradé subtil
 * - **Métriques claires** : entrées / sorties / solde net avec tendance en % (via `caisseTrends`)
 * - **Bouton "Voir plus"** en haut à droite (remplace "+ Nouveau"), aligné avec la charte
 * - **Indicateurs de tendance** : flèches vertes/rouges sur les métriques (ex: +12% vs mois dernier)
 * - **Effet de survol amélioré** : transition douce, opacité, focus visible
 * - **Design ultra compact** : padding réduit, espacement optimisé, parfait pour colonne 2
 * - **Illustration de fond** : micro‑barres décoratives en bas (optionnelle)
 * - **État vide** avec illustration et message dynamique
 * - **Accessibilité** : rôles ARIA, navigation clavier, skeleton précis
 *
 * @example
 * ```tsx
 ...
 *   caisseTrends={{ entreesMois: 5.2, sortiesMois: -2.1 }}
 *   maxItems={5}
 *   isLoading={false}
 *   onViewAll={() => router.push('/caisse')}
 *   onViewMouvement={(m) => router.push(`/caisse/${m.id}`)}
 * />
 * ```
 *
 * @author Stive Junior
...
function formatFCFA(num: number): string {
...
/** Avatar moderne avec dégradé et icône */
...
export function CaisseMouvementsRecentCard({
  ...
  const mouvementsVisibles = useMemo(() => mouvements.slice(0, maxItems), [mouvements, maxItems]);

  const soldeNet = useMemo(() => {
    if (!caisseStats) return 0;
    return caisseStats.entreesMois - caisseStats.sortiesMois;
  }, [caisseStats]);
      ...
      >
        ...
        <div className="px-1 pt-5 sm:px-6 sm:pt-6">
          <div className="flex items-center justify-between gap-3">
            ...
            {/* Bouton "Voir plus" (style StatsCard) */}
          ...
          {/* Métriques (entrées, sorties, solde net) */}
          {!isLoading && caisseStats ? (
            <div className="mt-5 flex flex-wrap justify-between gap-3 border-t border-border/30 pt-4">
              <MetricItem
                label="Entrées (mois)"
              ...
              />
              <div className="w-px bg-border/40 h-8" />
              <MetricItem
                label="Sorties (mois)"
              ...
              <MetricItem
                label="Solde net"
                value={`${soldeNetPositif ? '+' : ''}${formatCompact(soldeNet)}`}
                icon={Wallet}
                valueColor={soldeNetPositif ? 'text-emerald-600' : 'text-red-600'}
              />
            </div>
          ) : isLoading ? (
        ...
        <div className="flex-1 px-5  sm:px-6">
          <ul className="divide-y divide-border/30">
            {isLoading ? (
              Array.from({ length: maxItems }).map((_, i) => <MouvementSkeleton key={i} />)
            ) : mouvements.length === 0 ? (
